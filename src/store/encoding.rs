...
/// Encode `value` as CBOR, appending to `out`.
///
/// Callers holding plaintext reserve `out` up front (see [`encoded_size`]) so
/// no reallocation leaves a copy behind.
pub(crate) fn encode_into<T: Serialize + ?Sized>(
    value: &T,
    out: &mut Vec<u8>,
) -> Result<(), EncodingError> {
    ciborium::into_writer(value, out).map_err(|e| EncodingError::Encode(e.to_string()))
}

/// Number of bytes [`encode`] produces for `value`, without allocating.
pub(crate) fn encoded_size<T: Serialize + ?Sized>(value: &T) -> Result<usize, EncodingError> {
    let mut counter = ByteCounter(0);
    ciborium::into_writer(value, &mut counter).map_err(|e| EncodingError::Encode(e.to_string()))?;
    Ok(counter.0)
}

/// Write sink that only counts the bytes passing through.
...
mod tests {
    ...
    fn encoded_size_matches_encode() {
        ...
        encode_into(&value, &mut out).unwrap();

        assert_eq!(out.len(), size);
        assert_eq!(out.capacity(), size, "encoding must not reallocate");
        assert_eq!(out, encode(&value).unwrap());
    }
